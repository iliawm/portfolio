      ...
      {apps.map((app, index) => {
        ...
        if (app.isOnDesktop)
          return (
            <motion.div
              key={app.id || index}
              className={`absolute px-2 hidden md:flex border-gray-300  flex-col items-center justify-center py-5 gap-1 cursor-pointer select-none m-2 active:cursor-grabbing active:scale-95 ${
                active ? "bg-white/20 border" : ""
              }`}
              style={{
                width: `${gridSize}px`,
                height: `${gridSize}px`,
                ...
                if (movingIds.length > 1) {
                  ...
                  const newPositions = movingIds.map((id) => {
                    ...
                    if (
                      ...
                      newRow < 0 ||
                      newRow >= maxRows ||
                      isOccupiedByStaticApp(newCol, newRow)
                    ) {
                      canMove = false;
                    }
                  ...
                  let finalCol = Math.max(
                    0,
                    Math.min(targetCol, maxCols - 1)
                  );
                  let finalRow = Math.max(
                    0,
                    Math.min(targetRow, maxRows - 1)
                  );
                  ...
                  if (isOccupiedByStaticApp(finalCol, finalRow)) {
                    ...
                    if (
                      rightCol < maxCols &&
                      !isOccupiedByStaticApp(rightCol, finalRow)
                    ) {
                      finalCol = rightCol;
                    } else {
                      ...
                      if (
                        leftCol >= 0 &&
                        !isOccupiedByStaticApp(leftCol, finalRow)
                      ) {
                        finalCol = leftCol;
                      } else {
                        ...
                        if (emptySlots.length > 0) {
                          const randomIndex = Math.floor(
                            Math.random() * emptySlots.length
                          );
                          finalCol = emptySlots[randomIndex].col;
                          finalRow = emptySlots[randomIndex].row;
                        } else {
                          window.alert(
                            "cant change position desktop full of apps"
                          );
                          return;
                        }
                      }
            ...
            >
              <span className="text-black xs:text-sm font-mono relative h-full w-full p-5 flex items-center justify-center text-2xl">
                {app.isIconpath ? (
                  <Image
                    ...
                    fill
                    sizes="auto"
                    className="px-2.5 py-0.5 object-contain"
                    loading="eager"
                    draggable={false}
                  />
                ) : (
              ...
              <span
                className={`font-black text-white [-webkit-text-stroke:0.7px_black] ${
                  length <= 8 ? "text-nowrap" : ""
                }`}
              >
                {app.name}
              </span>
