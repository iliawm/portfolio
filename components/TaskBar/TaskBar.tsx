...
const TaskBar = () => {
    ...
    const renderMenu = () => {
        switch (menuIndex) {
            case 0:
                return <div className="p-4"><MainMenu/></div>
            case 1:
                return <div className="p-4"><SearchMenu/></div>
            default:
                return <div className="p-4"><MainMenu/></div>
        }
    }
    ...
    return (
        <section className={`w-full h-15 ${currentTheme === "dark" ? "bg-[#1A1A1A]" : "bg-[#FFFFFF]"} fixed bottom-0 flex z-40 justify-center items-center py-2 gap-2 invisible md:visible`}>
            <div className={`absolute h-140 w-120 lg:w-170 lg:h-170 ${theme==="dark" ? "bg-[#1A1A1A]":"bg-white"} transition-all bottom-19 z-30 rounded-xl ${Menu ? "opacity-100 " : "translate-y-full pointer-events-none no opacity-0"}`}>
                {renderMenu()}
            </div>

            {/* WINDOWS */}
            <button className={` cursor-pointer w-12 h-full relative ${theme==="dark"?"hover:bg-gray-600":"hover:bg-gray-200"} rounded-lg hover:scale-[1.1] active:scale-[1]`} onClick={() => {
                if (menuIndex !== 0 && Menu) {
                    SetMenu(true)
                    SetMenuIndex(0)
                } else {
